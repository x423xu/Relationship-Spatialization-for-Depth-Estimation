...
def build_data_loader(cfg, split="train", num_im=-1, is_distributed=False, start_iter=0):
    ...
    if cfg.DATASET.NAME == "vg" and cfg.DATASET.MODE == "benchmark":
        ...
        return dataloader
    elif cfg.DATASET.NAME == 'nyuv':
        transforms  = build_transforms(cfg, is_train=True if split=="train" else False, nyuv=True)
        dataset = nyuv(split=split, transforms=transforms, num_im = num_im)

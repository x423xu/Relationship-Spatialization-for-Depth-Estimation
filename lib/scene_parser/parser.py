...
class SceneParser(GeneralizedRCNN):
    ...
    def forward(self, images, id=None, split = None, targets=None):
        """
        Arguments:
        ...
        if self.roi_heads:
            ...
            if self.rel_heads:
                ...
                if id:
                    roi_features = x[0].detach().cpu().numpy()
                    rel_features = x[1].detach().cpu().numpy()
                    detections_save = {
                        'bbox': result[0][0].bbox.detach().cpu().numpy(),
                        'labels':result[0][0].get_field('labels').detach().cpu().numpy(),
                        'scores':result[0][0].get_field('scores').detach().cpu().numpy(),
                        'logits':result[0][0].get_field('logits').detach().cpu().numpy(),
                        'features':result[0][0].get_field('features').detach().cpu().numpy()
                        }
                    detection_pairs_save = {
                        'bbox': result[1][0].bbox.detach().cpu().numpy(),
                        'idx_pairs':result[1][0].get_field('idx_pairs').detach().cpu().numpy(),
                        'scores':result[1][0].get_field('scores').detach().cpu().numpy()
                        }
                    save_to_disk = {'roi_features':roi_features,
                                    'rel_features':rel_features,
                                    'detections':detections_save,
                                    'detection_pairs':detection_pairs_save}
                    id = id[0].decode('utf-8').split('.png')[0]
                    file_to_write = open('../../data/NYUv2/image/{}/{}.pkl'.format(split,id), "wb")
                    pickle.dump(save_to_disk,file_to_write)
                    file_to_write.close()
        else:
